...
const ContasReceber = ({ user, onLogout }) => {
  ...
  const fetchCategorias = async () => {
    try {
      ...
      setCategorias(response.data.receber || []);
    } catch (error) {
  ...
  const fetchContas = async () => {
    ...
    try {
      let url = `/contas/receber?company_id=${company.id}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingConta) {
        await axiosInstance.put(`/contas/receber/${editingConta.id}`, data);
        toast.success('Conta atualizada!');
      } else {
        await axiosInstance.post('/contas/receber', data);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axiosInstance.delete(`/contas/receber/${id}`);
  ...
  const handleMarcarRecebido = async (conta) => {
    try {
      await axiosInstance.patch(`/contas/receber/${conta.id}/status`, {
        status: 'RECEBIDO',
  ...
  const handleMarcarRecebidoEmLote = async () => {
    ...
    try {
      const promises = selectedContas.map((id) =>
        axiosInstance.patch(`/contas/receber/${id}/status`, {
          status: 'RECEBIDO',
          data_pagamento: new Date().toISOString().slice(0, 10),
        })
      );
      ...
      toast.success(`${selectedContas.length} conta(s) marcada(s) como recebida(s)!`);
      setSelectedContas([]);
      fetchContas();
    } catch (error) {
  ...
  const getStatusBadge = (status) => {
    ...
    const icons = {
      ...
      RECEBIDO: <CheckCircle className="w-3 h-3 mr-1" />,
  ...
  return (
    <div className="flex min-h-screen bg-zinc-950 text-white">
      <Sidebar user={user} onLogout={onLogout} activePage="contas-receber" />
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-zinc-400 mt-1">Gerencie seus recebimentos e contratos</p>
            </div>
            <Button
              onClick={() => {
                resetForm();
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="bg-zinc-900 border-zinc-800">
              <CardHeader>
                <CardTitle className="text-sm font-medium text-zinc-400">Recebido</CardTitle>
              </CardHeader>
              <CardContent>
                ...
                <p className="text-xs text-zinc-500 mt-1">
                  {filteredContas.filter((c) => c.status === 'RECEBIDO' || c.status === 'PAGO').length} conta(s)
                </p>
              </CardContent>
            </Card>
            ...
            <CardContent className="pt-6">
              ...
              {selectedContas.length > 0 && (
                <div className="mt-4 flex items-center gap-4">
                  ...
                  <Button onClick={handleMarcarRecebidoEmLote} size="sm" className="bg-green-600 hover:bg-green-700">
